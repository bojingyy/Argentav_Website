...
export default function TopBar({ onHomeSectionClick }) {
  ...
  const renderHomeLink = ({ id, label }, index) => {
    const className = `${index > 0 ? "mt-1 " : ""}block rounded-lg px-3 py-2 text-slate-200 transition hover:bg-slate-800 hover:text-white`;
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-slate-700 bg-black/90 shadow-[0_1px_12px_rgba(0,0,0,0.4)] backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-6 py-3 md:flex-row md:items-center md:justify-between lg:px-8">
        <div className="flex items-center gap-3 overflow-visible">
          ...
          <div className="ml-1 font-['Sora'] text-lg font-semibold tracking-[0.06em] text-white md:ml-2 md:text-2xl">Argentav</div>
        </div>
        <nav className="flex flex-wrap gap-2 text-xs md:text-sm">
          <div className="group relative">
            <Link
              ...
              className="flex cursor-default items-center gap-1 rounded-xl bg-blue-900 px-4 py-2 font-semibold text-white transition hover:translate-y-[-1px] hover:bg-blue-800"
            >
              Home
            ...
            <div className="pointer-events-none absolute left-0 top-full z-50 pt-2 opacity-0 transition group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100">
              <div className="min-w-[220px] rounded-xl border border-slate-700 bg-slate-950/95 p-2 shadow-[0_8px_24px_rgba(0,0,0,0.45)] backdrop-blur-md">
